...
class TheseusInsightPodcastGenerator:
    def __init__(
        self,
        text_model: dict = {
...
No other text outside JSON. There are only two speakers on the podcast: speaker-1 and speaker-2.
    ...
    def generate_podcast(
        ...
        theseus_insight_sections: List[str] | str,
        ...
        """
        Generates a podcast from a list of PDF files and TheseusInsight sections.

        This method processes a list of PDF files, extracts the text, and 
        generates a podcast script based on the provided TheseusInsight sections. 
        ...
        Args:
            ...
            theseus_insight_sections (List[str] | str): A list of TheseusInsight sections or a single TheseusInsight section.
        ...
        if isinstance(theseus_insight_sections, list):
            theseus_insight_sections = "\n".join(theseus_insight_sections)
        if verbose:
...
class GeneralPodcastGenerator:
    ...
    def _get_podcast_description(self, text: str) -> str:
        ...
        try:
            ...
            return "A podcast episode generated by TheseusInsight."
    ...
    def generate_podcast(
        ...
        try:
            ...
            for i, text in enumerate(texts):
                dialogue = self._get_dialogue(text=text, template_instructions= self.section)
                sections.append(dialogue)

            # 4) Create podcast introduction
            introduction = self._get_dialogue(text="\n".join(summaries), template_instructions=self.intro)
            ...
            outro = self._get_dialogue(text=str(merged_dialogue), template_instructions=self.outro)
