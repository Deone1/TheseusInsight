...
def construct_email_body(content,
    ...
    body = f"""## TheseusInsight Newsletter for {date_range}:
...
class GmailCommunication:
    ...
    def compose_message(self, content, start_date, end_date):
        ...
        if "Bcc" in message:
            del message["Bcc"]
        message['Subject'] = f" TheseusInsight Newsletter for {date_range}"
    ...
    def send_error_notification(self, error_msg: str):
        ...
        try:
            ...
            message['Subject'] = "TheseusInsight Error Notification"
            
            error_content = f"""## TheseusInsight Error Report

An error occurred during the TheseusInsight execution:
